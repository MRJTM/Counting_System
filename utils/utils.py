...
def _gather_feature(feat, ind, mask=None):
  # feat input size=[n,h*w,c], ind input size=[n,10000]
  if len(ind.shape)>2:
    ind=ind.squeeze(0)
  dim = feat.size(2)
  ind = ind.unsqueeze(2).expand(ind.size(0), ind.size(1), dim)  # [n,10000,2]
  feat = feat.gather(1, ind)  # [n,h*w,2],
  if mask is not None:
...
def load_model(model, pretrain_dir,map_location='cuda:0'):
