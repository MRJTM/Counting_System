...
def draw_umich_gaussian(heatmap, center, radius, k=1,overlap='add'):
    diameter = 2 * radius + 1
    ...
    if min(masked_gaussian.shape) > 0 and min(masked_heatmap.shape) > 0:  # TODO debug
        if overlap=='add':
            masked_gaussian/=np.sum(masked_gaussian)
...
# resize img
def resize_img_with_short_len(img, short_len=512):
    res_img = img.copy()
    height, width, _ = img.shape
    if height < width:
        h1 = short_len
        w1 = int(short_len / height * width)
        if w1 - w1 // 128 * 128 < 64:
            w1 = w1 // 128 * 128
        else:
            w1 = w1 // 128 * 128 + 128
        ...
        w1 = short_len
        h1 = int(short_len / width * height)
        if h1 - h1 // 128 * 128 < 64:
            h1 = h1 // 128 * 128
        else:
            h1 = h1 // 128 * 128 + 128
    res_img = cv2.resize(res_img, (w1, h1))
    return res_img
...
def img_to_tensor(img):
    ...
    mean = np.array(COCO_MEAN, dtype=np.float32)[None, None, :]
    std = np.array(COCO_STD, dtype=np.float32)[None, None, :]
    img -= mean
    img /= std
